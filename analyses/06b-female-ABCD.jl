...
# ============================================================================
# 06b-female-ABCD.jl
# Sex-stratified: FEMALE subjects only
# ============================================================================

commonFiles = [
    "output/openTotalFullDatamcmc-female/mcmc1.jld2",
    "output/openTotalFullDatamcmc-female/mcmc2.jld2",
]
stdFile = "output/stdPPmx-female/stdPPmxTot.jld2"
...
@info "Female-only filter: $(nrow(fullDf)) rows"
...
# ---- female not in covariate set (constant = 1 within subset) ----
...
mkpath("output/baselines-female")
CSV.write("output/baselines-female/frftotalComparison.csv", comparison)
println(comparison)

# plots: ARI vs RMSE
scatter(comparison.testARI, comparison.testRMSE,
        group = comparison.model, legend = :bottomleft, xlabel = "test ARI (FRF)",
        ylabel = "test RMSE", title = "FRF-total (female only): baselines vs PPMx")
Plots.savefig("output/baselines-female/frftotalComparison.png")

println("Done baselines comparison (female only); outputs in output/baselines-female/")
