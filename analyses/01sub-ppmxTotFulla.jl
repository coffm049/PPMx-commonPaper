...
Xtrain = convert(Matrix{Float64}, Matrix(train[:, modelVars]) .* 1.25)
Xtrain = hcat(ones(size(Xtrain)[1], 1), Xtrain)

Xtest = convert(Matrix{Float64}, Matrix(test[:, modelVars]) .* 1.25)
...
y1 = convert(Vector{Float64}, train[:, :nTotal]) .* 8 
y2 = convert(Vector{Float64}, test[:, :nTotal]) .* 8
...
@save "output/openTotalFull1.jld2" sim model
sim = mcmc!(model, 6000; mixDPM=true)
@save "output/openTotalFull2.jld2" sim model
