...
# ============================================================================
# 05c-male-ppmxStd-ABCD.jl
# Sex-stratified: MALE subjects only. Standard PPMx (no common-effect model).
# ============================================================================
...
Xtest = convert(Matrix{Float64}, Matrix(test[:, modelVars]))
Xtest = hcat(ones(size(Xtest)[1], 1), Xtest)
ytrain = convert(Vector{Float64}, train[:, :nTotal])
ytest = convert(Vector{Float64}, test[:, :nTotal])
...
CSV.write("output/stdPPmx-male/betas.csv",
...
println("Done standard PPMx fit (male only)")
